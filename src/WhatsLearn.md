...
* EX)
* localhost:3000/home => Home 컴포넌트 호출
* localhost:3000/about => about 컴포넌트 호출

* `Route`에는 2가지 `props`를 전달할 수 있는데 하나는 URL에 맞는 컴포넌트를 불러 주기 위한 `path props`이고, 
* 하나는 URL에 맞는 컴포넌트를 불러 주기 위한 `component porps`이다.
* path, component props를 통해 사용자가 접속한 URL을 보고, 그에 맞는 컴포넌트를 화면에 그릴 수 있게 된다.
